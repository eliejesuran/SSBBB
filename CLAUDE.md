...
- **Export PDF** : pagination automatique A4, suit le thème actif — **mode sombre** : blanc sur noir (variables `--black`/`--white`) ; **mode clair** : forcé en **noir sur blanc** (et non sur fond sépia) pour faciliter l'impression (`isLight` ⇒ `pdfBg=#fff`, `pdfText=#1a1a1a`, bordures/gris neutres). Badges via les couleurs réellement calculées du thème (`pdfBadgeColors`) ; cadre de notes inclus ; nom de fichier avec l'année
